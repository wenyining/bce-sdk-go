...
func UpdateAppServerGroupPort() {
	ak, sk, endpoint := "Your AK", "Your SK", "Your endpoint"

	blbClient, _ := appblb.NewClient(ak, sk, endpoint) // 初始化client

	BlbID := "Your app blb's id"
	UpdateAppServerGroupPortArgs := &appblb.UpdateAppServerGroupPortArgs{
	...
	err := blbClient.UpdateAppServerGroupPort(BlbID, UpdateAppServerGroupPortArgs)
	if err != nil {
		fmt.Println("update app server group port error : ", err)
		return
	}
}
