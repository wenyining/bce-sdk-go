...
func CreateAppHTTPListener() {
	ak, sk, endpoint := "Your AK", "Your SK", "Your Endpoint"

	appBlbClient, _ := appblb.NewClient(ak, sk, endpoint) // 初始化client

	blbID := "Your BlbID" // blb实例ID
	createAppHTTPListenerArgs := &appblb.CreateAppHTTPListenerArgs{
		ListenerPort: 8080,         // 监听端口
		Scheduler:    "RoundRobin", // 调度算法
	}

	// 创建appblb http监听器
	if err := appBlbClient.CreateAppHTTPListener(blbID, createAppHTTPListenerArgs); err != nil {
		panic(err)
	}
}
