...
func CreateAppIpGroupBackendPolicy() {
	ak, sk, endpoint := "Your AK", "Your SK", "Your endpoint"

	blbClient, _ := appblb.NewClient(ak, sk, endpoint) // 初始化client

	BlbID := "Your app blb's id"
	CreateAppIpGroupBackendPolicyArgs := &appblb.CreateAppIpGroupBackendPolicyArgs{
	...
	err := blbClient.CreateAppIpGroupBackendPolicy(BlbID, CreateAppIpGroupBackendPolicyArgs)
	if err != nil {
		fmt.Println("create app ip group backend policy error : ", err)
		return
	}
}
