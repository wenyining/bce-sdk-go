...
func UnbindSecurityGroups() {
	...
	args := &appblb.UpdateSecurityGroupsArgs{
		// 设置要解绑的 security group 列表
		SecurityGroupIds: []string{"g-wrukksxxxxz5"}, // 替换为需要解绑的 security group 列表
	}

	if err := appblbClient.UnbindSecurityGroups(AppBlbID, args); err != nil {
