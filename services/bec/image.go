...
// ImportImage - import a image
//
// PARAMS:
//   - args: the import image args
//
// RETURNS:
//   - *api.CreateVmImageResult: the result image
//   - error: nil if ok otherwise the specific error
func (c *Client) ImportImage(args *api.ImportImageArgs) (*api.CreateVmImageResult, error) {
	if args == nil {
		return nil, fmt.Errorf("please set argments")
	}

	result := &api.CreateVmImageResult{}
	req := &api.PostHttpReq{Url: api.GetImportImageURI(), Result: result, Body: args}
	err := api.Post(c, req)
